...
collisions.run6<-collisions[128:139,]
...
colliders.run6<-colliders$V2[128:139]
...
mass.run.8<-as.numeric(as.vector(collisions.run8$xsa))
hist(mass.run.8,col=rgb(1,0,0,0.5),xlab="object mass",ylim=c(0,25),main="distribution of impactor and target masses for run 8")
hist(colliders.run8,col=rgb(0,0,1,0.5),add=T)
